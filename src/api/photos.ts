...
export const photosApi = {
  ...
  upload: (albumId: string, file: File, onProgress?: (value: number) => void) => {
  ...
  reorder: (albumId: string, orderedPhotoIds: string[]) => api.patch<Photo[]>(`/albums/${albumId}/photos/reorder`, { orderedPhotoIds }).then((r) => r.data),
  remove: (id: string) => api.delete<void>(`/photos/${id}`).then((r) => r.data),
};
