...
export function useUploadPhoto(albumId: string) {
  ...
  return useMutation({ mutationFn: ({ file, onProgress }: { file: File; onProgress?: (value: number) => void }) => photosApi.upload(albumId, file, onProgress), onSuccess: (photo) => {
    client.setQueryData<Photo[]>(queryKeys.photos(albumId), (current = []) => {
      const withoutDuplicate = current.filter((item) => item.id !== photo.id);
      return [...withoutDuplicate, photo].sort((a, b) => a.position - b.position);
    });
    client.setQueryData<AlbumDetail>(queryKeys.album(albumId), (current) => current ? {
      ...current,
      _count: { photos: current._count.photos + 1 },
    } : current);
    client.setQueryData<AlbumListItem[]>(queryKeys.albums, (current) => current?.map((album) => album.id === albumId ? {
      ...album,
